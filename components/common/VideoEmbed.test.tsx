...
describe('VideoEmbed', () => {
  ...
  describe('네이티브 비디오 (F043)', () => {
    it('.mp4 URL이면 <video> 태그를 렌더링한다', () => {
      render(<VideoEmbed url="/videos/layup.mp4" title="레이업 영상" />)
      const video = screen.getByRole('region', { name: '레이업 영상' })
      const source = video.querySelector('source')
      expect(source).toHaveAttribute('src', '/videos/layup.mp4')
      expect(source).toHaveAttribute('type', 'video/mp4')
    })

    it('.webm URL이면 <video> 태그를 렌더링한다', () => {
      render(<VideoEmbed url="/videos/drill.webm" title="드릴 영상" />)
      const video = screen.getByRole('region', { name: '드릴 영상' })
      const source = video.querySelector('source')
      expect(source).toHaveAttribute('src', '/videos/drill.webm')
      expect(source).toHaveAttribute('type', 'video/webm')
    })

    it('YouTube URL이면 기존 iframe을 유지한다', () => {
      render(<VideoEmbed url="https://youtube.com/watch?v=dQw4w9WgXcQ" title="유튜브" />)
      expect(screen.getByTitle('유튜브').tagName).toBe('IFRAME')
    })

    it('poster prop이 있으면 <video poster> 속성을 설정한다', () => {
      render(
        <VideoEmbed url="/videos/layup.mp4" title="포스터 테스트" poster="/images/poster.jpg" />
      )
      const video = screen.getByRole('region', { name: '포스터 테스트' })
      const videoEl = video.querySelector('video')
      expect(videoEl).toHaveAttribute('poster', '/images/poster.jpg')
    })
  })
})
